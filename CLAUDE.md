...
`.env` 里**只有三个**（会真的按需调整的）：`PROMPT_MAX_TOKENS`（8192，与思考共享额度，截断报错文案直接指向它）/ `PROMPT_THINKING`（off）/ `PROMPT_TIMEOUT`（180，**`tagger.py` 的 VLM 路径没传 timeout，这里必须显式传**）。端点复用 `VISION_API_URL` / `VISION_API_KEY` / `VISION_MODEL`。
...
- **翻译列刷新点共四处**：切图（`loadCaption`）、进度弹窗关闭（`refreshCurrentImage`）、标签被编辑（`onTagChanged`）、保存成功（`refillTranslations`）。`refillTranslations` 只补「标签非空且翻译为空」的行、只打 `/lookup_cache`（纯查库，不触发 LLM，也没有收录/翻译新标签的能力），响应到达后二次校验行标签未变且仍为空才写 DOM——否则会覆盖用户刚编辑出来的结果。**翻译列空白 ≠ 显示 bug**：主库未收录（如 meta 类 `masterpiece`/`best quality`，或非 Danbooru 标签）的标签在任何地方都没有翻译可显示，编辑页也没有给它们留出路（`translateTagWiki` 无 en_wiki 时直接提示；`/translate_single_tag` 对未收录标签返回 404；页面未接 `/user_tags`），录入只能去 Danbooru 查询页的新标签入口
...
`_call_llm` 显式传 `max_tokens`（`.env` 的 `LLM_MAX_TOKENS`，默认 8192）——思考与正式回答共享该额度，省略则走服务端默认值，思考占满后返回空 `content`。`content` 为空且 `finish_reason='length'` 时抛 `_OutputTruncated`（`ValueError` 子类，路由的 `except ValueError` 能接住转成 400），与上下文超限一样走**拆半递归重试**，仍失败才显式报错——**不要退回静默返回 `[]`**，那会让调用方以为「这批本来就没结果」，整批白跑、不记历史、一轮轮重试且日志里看不出异常。
