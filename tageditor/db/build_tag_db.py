...
def full_scan_wiki(db_path, verbose=True, progress_callback=None, cancel_check=None):
    ...
    from tageditor.core.config import get_danbooru_config
...
def main():
    ...
    if args.cmd == 'init':
        ...
        from tageditor.db.sync_tags import run as run_sync_tags
        run_sync_tags(db_path, download=args.download)
        show_stats(db_path)
    elif args.cmd == 'tag-groups':
        from tageditor.db.tag_groups import run as run_tag_groups
        run_tag_groups(db_path)
    elif args.cmd == 'llm-process':
        from tageditor.translate.llm_pipeline import run_llm_process
        ...
        from tageditor.db.tag_groups import run as run_tag_groups
        from tageditor.translate.llm_pipeline import run_llm_process
        from tageditor.db.cooc_pipeline import run_fetch_cooc, run_trim_cooc
        log.info('=' * 60)
