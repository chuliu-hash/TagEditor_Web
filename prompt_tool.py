...
# tag_groups 单次返回的成员预算（防止 people 这种 1831 人的大组撑爆 payload）
...
def _batch_cn_first_segment(conn, terms, per_term=8):
    ...
    一次全表扫（实测 5 词 107ms）。**不用 search_tags**：它的 MATCH 里 OR 了
    other_names，中文被别名污染（搜 '白发' 首位是 red_eyes，'和服' 混进 leotard）。
    cn_name 首段同时兼容半角/全角逗号（实测 1girl = 单人少女,单人女性，一位女孩，独居少女）。
    """
    terms = [t for t in dict.fromkeys(terms) if t]
    if not terms:
        return {}
    # 每个词一组 (精确 | 半角逗号后接 | 全角逗号后接)，组间 OR —— 一次全表扫覆盖全部关键词
    clause = "(cn_name = ? OR cn_name LIKE ? ESCAPE '\\' OR cn_name LIKE ? ESCAPE '\\')"
    params = []
    for t in terms:
        esc = _esc_like(t)
        params.extend([t, f'{esc},%', f'{esc}，%'])
...
def execute_tool_calls(conn, calls, cfg, warnings, exclude, allowed):
    ...
    for name, raws in pending.items():
        ...
        try:
            ...
            print(f"[PromptTool] 工具 {name} 执行失败: {e}")
...
def _encode_image(file_path, cfg, warnings):
    ...
    try:
        ...
        print(f"[PromptTool] 图片尺寸检查跳过（Pillow 不可用）: {e}")
    if not need_shrink:
    ...
    try:
        ...
        warnings.append(f'图片压缩失败（{e}），按原图发送')
        return base64.b64encode(raw).decode('ascii'), mime
...
def _completion_text(resp):
    ...
    if finish == 'length':
        raise _PromptFatal('模型输出被 max_tokens 截断（思考与正式回答共享额度）：'
                           '请调大 PROMPT_MAX_TOKENS 或减少输入标签')
    if reasoning:
        raise _PromptFatal(f'模型把额度花在思考上没有正式回答（finish={finish}）：'
                           f'请确认 PROMPT_THINKING=off')
...
def _call_vlm_json(image_b64, mime, payload, cfg):
    ...
    try:
        ...
        print("[PromptTool] 端点不支持 response_format，降级重试")
    ...
    for item in raw:
        ...
        if op == 'remove':
            ...
            if base['op'] == 'modify':
                pass
            base['tag'] = new_tag
            base['weight'] = item.get('weight', base.get('weight'))
            base['reason'] = reason or base.get('reason') or ''
...
def prompt_adjust():
    ...
    def _run(cancel_evt):
        ...
        try:
            ...
            planner_ok = False
            print(f"[PromptTool] 规划轮失败，降级为关键词直检: {e}")
        ...
        if use_db:
            ...
            for item in (parsed_model.get('diff') or []):
                if isinstance(item, dict) and (item.get('op') or '').lower() == 'add' and item.get('tag'):
                    diff.append({'op': 'add', 'tag': item['tag'], 'weight': item.get('weight'),
